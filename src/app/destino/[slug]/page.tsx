      ...
      <div className="w-full max-w-[1200px] mx-auto px-4 mt-6">
        <div className="flex flex-col gap-6">
          ...
          <div className="grid gap-4 bg-white border border-border rounded-[22px] p-5 shadow-sm" style={{ gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))' }}>
            <div>
              <div className="text-[0.75rem] uppercase text-text-muted font-bold flex items-center gap-1">
                <MapPin size={13} className="text-rojo" /> Dirección
              </div>
          ...
          <div className="bg-white border border-border rounded-2xl overflow-hidden shadow-sm p-6">
            <h2 className="font-display font-bold text-[1.3rem] text-text-primary mb-3 flex items-center gap-2">
              <BookOpen size={20} className="text-rojo" /> Reseña del Destino
            </h2>
            ...
            {destination.historia && (
              <div className="mt-5 pt-5 border-t border-border">
                <h3 className="font-display font-semibold text-[1.125rem] text-text-primary mb-2 flex items-center gap-2">
                  <Landmark size={18} className="text-sol-dark" /> Historia en Pelotillehue
                </h3>
          ...
          {destination.galeria && destination.galeria.length > 0 && (
            <div className="bg-white border border-border rounded-2xl overflow-hidden shadow-sm p-6">
              <h2 className="font-display font-bold text-[1.3rem] text-text-primary mb-4 flex items-center gap-2">
                <ImageIcon size={20} className="text-rojo" /> Galería de Fotografías
              </h2>
              <div className="grid gap-3" style={{ gridTemplateColumns: 'repeat(auto-fill, minmax(180px, 1fr))' }}>
                {destination.galeria.map((imgUrl, idx) => (
                  <div key={idx} className="rounded-xl overflow-hidden h-[140px] bg-surface-soft border border-border">
                    <img
          ...
          <div className="bg-white border border-border rounded-2xl overflow-hidden shadow-sm p-6">
            <h2 className="font-display font-bold text-[1.3rem] text-text-primary mb-2 flex items-center gap-2">
              <Navigation size={20} className="text-rojo" /> Cómo Llegar
            </h2>
          ...
          {related.length > 0 && (
            <div className="mt-4">
              <h3 className="font-display font-bold text-[1.3rem] text-text-primary mb-3 flex items-center gap-2">
                <Sparkles size={20} className="text-rojo" /> Otros atractivos en Cumpeo
              </h3>
              <div className="grid gap-4" style={{ gridTemplateColumns: 'repeat(auto-fill, minmax(240px, 1fr))' }}>
                {related.map((r) => (
                  <div key={r.id} className="bg-white border border-border rounded-2xl overflow-hidden shadow-sm hover:border-rojo hover:shadow-md hover:-translate-y-1 transition-all cursor-pointer">
